 ...
 * @param {number} args.amount - Amount of times to call the check before failing
 * @param {number} args.wait - Time to wait between each check
 * @param {Array} [...args] - Arguments to pass to the check, and onFinish methods
 *
 * @returns {Promise}
 */
const waitForIt = ({ check, onFinish, amount=4, wait=1000, total }, ...args) => {
  total = total || 0
  return new Promise(async (res, rej) => {
    total++
