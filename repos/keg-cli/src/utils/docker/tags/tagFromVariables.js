...
/**
 * Map of shortcuts matching the allowed shortcuts from the task caller tag-variable option
 * Maps each shortcut to a key name, that is then mapped to a method
 * @type Object
 */
const variableMap = {
  'b': 'branch',
...
const getVariableValue = (vars, methodArgs) => {
  return vars.reduce(async (toResolve, placeholder) => {
    ...
    const method = variableMap[placeholder]
