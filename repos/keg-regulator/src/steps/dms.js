...
When(/^on the login form the user enters \"(.*?)\" in the email field$/, { timeout }, function(user_email) {
  return this.driver.wait(until.elementLocated({xpath: "/html/body/div/div[2]/div[2]/div[1]/form/div[1]/div[1]/div/div/input"}), 10000).then(element => element.sendKeys(user_email));
});

//Enter password
When(/^on the login form the user enters \"(.*?)\" in the password field$/, { timeout }, function(user_password) {
  return this.driver.wait(until.elementLocated({xpath: "/html/body/div/div[2]/div[2]/div[1]/form/div[1]/div[2]/div/div/input"}), 10000).then(element => element.sendKeys(user_password));
});

//Login form - Submit
When(/^on the login form the user submits the login form$/, { timeout }, function() {
  return this.driver.wait(until.elementLocated({xpath: "/html/body/div/div[2]/div[2]/div[1]/form/div[2]/div[1]/div/button"}), 10000).then(element => element.click());
});
