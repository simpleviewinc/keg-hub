...
var validateArguments = function validateArguments() {
  ...
  if (!_rollupPluginBabelHelpers.hasDomAccess() || !jsutils.isArr(args.classNames)) {
  ...
  return _rollupPluginBabelHelpers._objectSpread2(_rollupPluginBabelHelpers._objectSpread2({}, args), {}, {
    callback: callback
  });
};
...
var getStyleContent = function getStyleContent(styleStr) {
  var matches = _rollupPluginBabelHelpers._toConsumableArray(styleStr.matchAll(/\{(.+?)\}/gi));
...
var cssToJs = function cssToJs(styleStr) {
  ...
  return styles.reduce(function (parsed, styleRule) {
    ...
    var _styleRule$split = styleRule.split(':'),
        _styleRule$split2 = _rollupPluginBabelHelpers._slicedToArray(_styleRule$split, 2),
        name = _styleRule$split2[0],
        value = _styleRule$split2[1];
    name = jsutils.camelCase(name.trim());
    value = value.trim();
    return !jsutils.exists(name) || !jsutils.exists(value) || name === '' || value === '' ? parsed : _rollupPluginBabelHelpers._objectSpread2(_rollupPluginBabelHelpers._objectSpread2({}, parsed), {}, _rollupPluginBabelHelpers._defineProperty({}, name, value));
  }, styleObj);
};
