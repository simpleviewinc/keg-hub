...
import { get, deepClone } from '@ltipton/jsutils'
...
describe('Theme', () => {
  describe('restructureTheme', () => {
    ...
    it('should update the theme to use the values of the current platform', () => {
      ...
      expect(get(testTheme, 'meeting.$xsmall.meetingList.$native')).not.toBe(
        undefined
      )
    ...
    it('should return an updated theme using the current platform values', () => {
      ...
      expect(get(testTheme, 'meeting.$large.text.$web.time.font')).toBe(
        'WEB FONT'
      )
    ...
    it('should allow passing custom platforms as the second argument', () => {
      ...
      const customFont = get(
        testTheme,
        'meeting.$large.text.$customPlatform.time.font'
      )
      ...
      const cutomTheme = Theme.restructureTheme(deepClone(testTheme), {
        $customPlatform: true,
      })
      expect(get(cutomTheme, '$large.meeting.text.time.font')).toEqual(
        customFont
      )
    })
  })
