...
describe('/url', () => {
  ...
  describe('isValidUrl', () => {
    it('should return TRUE for valid urls', () => {
      const urls = ["https://google.com?name=daniel&id=1", "http://google.com????", "https://www.google.uk", "https://zsales.zerista.com"];
      urls.map(url => {
        expect(Url.isValidUrl(url)).toBe(true);
      });
    });
    it('should return FALSE for invalid urls', () => {
      const urls = ["hasdfas://google.com?name=daniel&id=1", "//google.com????", "htp://www.google.uk", "zsales.zerista.com"];
      urls.map(url => {
  ...
  describe('objToQuery', () => {
    ...
    it('return a valid querystring from the given object with nested object', () => {
      ...
      const result = Url.objToQuery(obj); // just appends the nested object via JSON string
