...
const getFiles = (searchLocation) => {
  return fs.readdirSync(searchLocation, { withFileTypes: true })
...
const assetFileNames = (tapAssetPath, extensions = []) => {
  ...
  const allExts = get(tapConstants, [ 'extensions', 'assets' ], noPropArr).concat(
    extensions
  )
  ...
  return Array.from(
    ...
    new Set(
      ...
      getFiles(tapAssetPath).filter(file => allExts.indexOf(path.extname(file)) !== -1)
    )
  )
}
...
const writeAssetFiles = args => {
  ...
  builtAssets.length &&
    fs.writeFileSync(
      path.join(assetsFull, ensureJsExt(assetIndex)),
      autoGeneratedText
        .concat(
          `\nexport const assets = {\n${builtAssets.join(',\n')}\n}\n`
        )
  ...
  const resp = builtFonts.length &&
    fs.writeFileSync(
      path.join(fontsFull, ensureJsExt(fontsIndex)),
      autoGeneratedText
        .concat(
          `\nexport const fonts = {\n${builtFonts.join(',\n')}\n}\n`
        )
...
module.exports = (appConf, BASE_PATH, TAP_PATH) => {
  ...
  ;(builtFonts.length || builtAssets.length) &&
    writeAssetFiles({
      ...
      fontsFull,
      assetsFull,
      fontsIndex: pathsConfig.fontsIndex || pathsConfig.folderRootFile || tapConstants.fontsIndex,
      assetIndex: pathsConfig.assetsIndex || pathsConfig.folderRootFile || tapConstants.assetsIndex,
    })
