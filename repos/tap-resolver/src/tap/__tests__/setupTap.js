...
describe('Setup Tap', () => {
  ...
  describe('Finding the correct base path', () => {
    it('should use the BASE_PATH set in the config if it is a directory', () => {
      validPathMatch = path.join(
        testAppRoot,
        get(config, [ 'keg', 'tapResolver', 'paths', 'kegSrc' ])
      )
  ...
  describe('Configuring the tap', () => {
    it('should indicate if a tap folder exists or not', () => {
      const { HAS_TAP } = setupTap({
        ...options,
        config: { ...config, name: config.keg.name },
      })
    ...
    it('should provide a merged config from the tap and the root path', () => {
      const { APP_CONFIG } = setupTap(options)
      ...
      expect(APP_CONFIG.keg.tapResolver.paths).toEqual(
        config.keg.tapResolver.paths
      )
    })
  })
})
