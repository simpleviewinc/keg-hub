...
describe('useQuery', () => {
  ...
  it('returns the value of the key', () => {
    Mocks.setMocks({
      components: { useLocation: mockLocation({ search: '?id=50' }) },
    })
  ...
  it('returns an array if search param is in array format', () => {
    Mocks.setMocks({
      components: { useLocation: mockLocation({ search: '?name=50,20,30' }) },
    })
  ...
  it('returns null on invalid/nonexistent key', () => {
    Mocks.setMocks({
      components: { useLocation: mockLocation({ search: '?id=50&type=foo' }) },
    })
    const obj = renderHook(() => useQuery('name'))
